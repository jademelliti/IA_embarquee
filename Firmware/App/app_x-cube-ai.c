...
/* USER CODE BEGIN includes */
 ...
 #define BYTES_IN_FLOATS 5*4 // 5 features en entrée :     Air temperature [K] Process temperature [K] Rotational speed [rpm] Torque [Nm] Tool wear [min]
 ...
 #define CLASS_NUMBER 5 //TWF,HDF,PWF,OSF et NF
...
#if !defined(AI_IA_EMBARQUE_INPUTS_IN_ACTIVATIONS)
AI_ALIGNED(4) ai_i8 data_in_1[AI_IA_EMBARQUE_IN_1_SIZE_BYTES];
ai_i8* data_ins[AI_IA_EMBARQUE_IN_NUM] = {
data_in_1
};
#else
ai_i8* data_ins[AI_IA_EMBARQUE_IN_NUM] = {
...
#if !defined(AI_IA_EMBARQUE_OUTPUTS_IN_ACTIVATIONS)
AI_ALIGNED(4) ai_i8 data_out_1[AI_IA_EMBARQUE_OUT_1_SIZE_BYTES];
ai_i8* data_outs[AI_IA_EMBARQUE_OUT_NUM] = {
data_out_1
};
#else
ai_i8* data_outs[AI_IA_EMBARQUE_OUT_NUM] = {
...
AI_ALIGNED(32)
static uint8_t pool0[AI_IA_EMBARQUE_DATA_ACTIVATION_1_SIZE];

ai_handle data_activations0[] = {pool0};

/* AI objects ----------------------------------------------------------------*/

static ai_handle ia_embarque = AI_HANDLE_NULL;
...
static int ai_boostrap(ai_handle *act_addr)
{
  ...
  err = ai_ia_embarque_create_and_init(&ia_embarque, act_addr, NULL);
  if (err.type != AI_ERROR_NONE) {
    ai_log_err(err, "ai_ia_embarque_create_and_init");
    return -1;
  }

  ai_input = ai_ia_embarque_inputs_get(ia_embarque, NULL);
  ai_output = ai_ia_embarque_outputs_get(ia_embarque, NULL);

#if defined(AI_IA_EMBARQUE_INPUTS_IN_ACTIVATIONS)
  /*  In the case where "--allocate-inputs" option is used, memory buffer can be
   *  used from the activations buffer. This is not mandatory.
   */
  for (int idx=0; idx < AI_IA_EMBARQUE_IN_NUM; idx++) {
   ...
   */
  for (int idx=0; idx < AI_IA_EMBARQUE_OUT_NUM; idx++) {
...
static int ai_run(void)
{
  ...
  batch = ai_ia_embarque_run(ia_embarque, ai_input, ai_output);
  if (batch != 1) {
    ai_log_err(ai_ia_embarque_get_error(ia_embarque),
        "ai_ia_embarque_run");
    return -1;
  }
